...
String GOOGLE_SCRIPT_ID = "AKfycbxGFKN7IZuP8r0uqwfUiLg0vvRY8AAcYrNeUGkNlVQ4Zt9MyMzPkAVZmfWr4OS9Of4u_A";
...
int handleNewMessages(int numNewMessages, String prev_text, int user_num) {
  ...
  for (int i=0; i<numNewMessages; i++) {
    ...
    else if (c_cmd == "/fund_transfer"){
      String dmssg = "Username amount \n";
      dmssg += "(Example :Press 2 for Rs 200)\n";
      bot.sendMessage(chat_id, dmssg, "");
      return user_num;
    }
    ...
    // to check the available balance present in the user's account
      ...
      String dem = " ";
      const char* demi = dem.c_str();
      char* check2;
      strcpy(check2,values.c_str());
      char * token = strtok(check2,demi);
      String u_nm = String(token);
      token = strtok(NULL, " ");
      String amt_f = String(token);
      f_amt = amt_f.toInt();
      if (WiFi.status() == WL_CONNECTED) {
        HTTPClient http;
        String url = "https://script.google.com/macros/s/" + GOOGLE_SCRIPT_ID + "/exec?user_row="+String(user_num);
    ...
    //displays the transaction summary in serial monitor as well as through the ATM Machine Bot if the credit transaction is successful.
      ...
      new_bal = bal-f_amt;
      if(new_bal>=0){
        String ctext="Transaction Successful. Fund Transferred to User "+String(u_nm)+"\n";
        ctext+="Transaction Details: \n";
        ctext+="Transaction Time : " +String(asString)+" \n";
        ctext+="Initial Balance : Rs "+String(bal) +" \n";
        ctext+="Transferred Amount : Rs "+String(f_amt) +" \n";
        ctext+="Remaining Balance : Rs "+String(new_bal) +" \n";
        Serial.println();
        Serial.println(ctext);
        bot.sendMessage(chat_id, ctext, "");
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      if(new_bal>=0){
        HTTPClient http;
        // To accesss the spreadsheet database of users
        String url = "https://script.google.com/macros/s/" + GOOGLE_SCRIPT_ID + "/exec?user_row_val2="+String(user_num)+"&amount2="+String(f_amt)+"&new_bal2="+String(new_bal)+"&ttime2="+String(asString) + "&u_nm="+String(u_nm);
        Serial.println("Making a request to spreadheet...");
        http.begin(url.c_str());
        http.setFollowRedirects(HTTPC_STRICT_FOLLOW_REDIRECTS);
        int user = http.GET();
